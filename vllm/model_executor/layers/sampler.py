...
class LogitProcessor(ABC):
    """Abstract class for logit processors"""
    ...
    def _apply_temperatures(
        ...
        temperatures = LogitProcessor._get_temperatures(sampling_metadata)
...
class DefaultLogitProcessor(LogitProcessor):
    """Default processor - Just applies temperature and returns probs."""
...
class Sampler(nn.Module):
    ...
    def __init__(self):
        ...
        self.processor = DefaultLogitProcessor()
    ...
    def forward(
        ...
        logits = self.processor.process(logits, sampling_metadata)
...
def _beam_search_sample(
    ...
    for seq_group in selected_seq_groups:
        ...
        if is_prompt:
            ...
            _, next_token_ids = torch.topk(
                seq_group_logprobs[0], 2 * beam_width
            )
            ...
            _, topk_ids = torch.topk(
                seq_group_logprobs.flatten(), 2 * beam_width
            )
    ...
    for sampling_type in SamplingType:
        ...
        if sampling_type == SamplingType.GREEDY:
            ...
            sample_results = _beam_search_sample(
                seq_groups, beam_search_logprobs
            )
...
def _sample_with_triton_kernel(
    ...
    for sampling_type in SamplingType:
        if sampling_type not in sample_metadata:
            continue
        (seq_group_id, seq_groups, sample_indices, sampled_token_indices) = (
            sample_metadata[sampling_type]
        )
        if sampling_type == SamplingType.GREEDY:
